 ...
 * @copyright 公司名称:Chuhee
 */
@SuppressWarnings("unchecked")
public class ESTransportUtils {

    private static final int PORT = 9300;// 端口
    public static TransportClient client;// 创建对象

    public ESTransportUtils(String clusterName, String ip){
        getClient(clusterName, ip);
    }
    /***
    * @Param  //参数
    ...
    */
    public static synchronized TransportClient getClient(String clusterName, String ip){
        ...
        return ESTransportUtils.client;
    }
    /***
    * @Param
...
//                        .put("client.transport.ping_timeout",3)
    ...
    */
     ...
     * @[java]return void
     */
    public synchronized void bulkInsertToES(String index, String type, Iterator<Object> objIter) {
        bulkInsertToES(index, type, objIter, null);
    }
     ...
     * @[java]return void
     */
    public synchronized void bulkInsertToES(String index, String type, Iterator<Object> objIter, String _id) {
        int batchSize = 1000;
        bulkInsertToES(index, type, objIter, _id, batchSize);
    }
     ...
     * @[java]return void
     */
    public synchronized void bulkInsertToES(String index, String type, Iterator<Object> objIter, String _id, int batchSize) {
        ...
        while (objIter.hasNext()) {
            ...
            bulkRequest(bulkRequest, index, type, _id, value, bean);
            count += 1;
     ...
     * @[java]return void
     */
    private synchronized void bulkRequest(BulkRequestBuilder bulkRequest, String index, String type, String _id, String value,Object bean) {
        if (_id != null) {
    ...
    /**
    * @@Param
    * @@[Java]Param [index, type, ids]
    * @@description 默认批次20提交查询，且不筛选列。
    * @@author luyj
    * @@date 2020/5/22 上午9:53
    * @@return
    * @@[java]return java.util.ArrayList<java.lang.String>
    */
    public synchronized ArrayList<String> mget(String index, String type, Iterable<String> ids){
        return mget(index, type, ids,new String[]{});
    }

    /**
    * @@Param
    * @@[Java]Param [index, type, ids, includes]
    * @@description 默认每个批次20，提交查询
    * @@author luyj
    * @@date 2020/5/22 上午9:50
    * @@return
    * @@[java]return java.util.ArrayList<java.lang.String>
    */
    public synchronized ArrayList<String> mget(String index, String type, Iterable<String> ids, String[] includes){
        return mget(index, type, ids,20, includes);
    }
    /**
    * @@Param
    * @@[Java]Param [index, type, ids, batchSize, includes]
    * @@description
    * @@author luyj
    * @@date 2020/5/22 上午9:37
    * @@return
    * @@[java]return java.util.ArrayList<java.lang.String>
    */
    public synchronized ArrayList<String> mget(String index, String type, Iterable<String> ids, int batchSize,String[] includes){
        ArrayList<String> list = new ArrayList<>(); //返回总的结果数组
        ArrayList<String> tmp = new ArrayList<>(); //存放临时批次
        Iterator<String> iter = ids.iterator(); //ids转换成iterator，用于使用hasnext()
        int cnt = 0;

        while (iter.hasNext()) {
            tmp.add(iter.next());
            cnt += 1;
            if (cnt % batchSize == 0 || !iter.hasNext()) {//达到批次阀值或消耗结束，执行查询ES
                list.addAll(mget(index, type, (Iterable) tmp,includes, new String[]{}));
//                System.out.println("batch: " + cnt +"info: "+tmp.toString());
                tmp.clear();//清理批次临时数据数组。
            }
        }
    ...
    /**
    * @@Param
    * @@[Java]Param [index, type, ids, includes, excludes]
    * @@description  筛选字段mget,根据ids数组整个一次请求。
    * @@author luyj
    * @@date 2020/5/21 下午1:39
    * @@return
    * @@[java]return java.util.ArrayList<java.lang.String>
    */
    public synchronized ArrayList<String> mget(String index, String type, Iterable<String> ids, String[] includes, String[] excludes){
        ArrayList<String> list = new ArrayList<>();//用于保存查询结果的json数据组
//        ArrayList<MultiGetRequest.Item> items = new ArrayList<>();
     ...
     */
    public synchronized ArrayList<String> searchTerms(String index, String type, Map<String, String> fieldMap) {
        ArrayList<String> list = new ArrayList<>();//用于保存查询结果的json数据组
        ...
        if (hits.totalHits() < 10) {//查询结果小于10条则，直接返回结果LIST
            ...
            list = searchAllTers(index, type, fieldMap);
        }
        return list;
    }
     ...
     * @[java]return java.util.List<java.lang.String> //java返回值
     */
    public synchronized ArrayList<String> searchAllTers(String index, String type, Map<String, String> fieldMap) {
        ArrayList<String> list = new ArrayList<>();//用于保存查询结果的json数据组
        BoolQueryBuilder bqb = QueryBuilders.boolQuery();//构建ES的bool查询
        for (String key : fieldMap.keySet()) {//根据查询的条件，构建bool查询的must描述
            TermQueryBuilder qb = QueryBuilders.termQuery(key, fieldMap.get(key));
     ...
     */
    public synchronized List<String> searchTermsPages(
            String index,
            String type,
...
//                .setPreference("_only_nodes:node02")
     ...
     * @[java]return java.util.List<java.lang.String>
     */
    public synchronized List<String> searchAllPages(
            String index,
            String type,
            Map<String, String> fieldMap,
            Integer from,
...
//            client.clearScroll()
    ...
    public synchronized List<String> searchTerm(String index, String type, String fieldKey, String fieldValue) {
    ...
    public synchronized Map<Object, String> searchTermsIDAndJson(String index, String type, int from, int size, Map<String, String> fieldMap) {
    ...
    public synchronized List<String> searchTerms( String index, String type, int from, int size, Map<String, String> fieldMap) {
    ...
    public synchronized String searchId( String index, String type, String id) {
...
//            client.prepareMultiGet().
    ...
    public synchronized GetResponse searchIdAsResponse( String index, String type, String id) {
    ...
    public synchronized boolean insertES( String index, String type, String id, String json) {
    ...
    public synchronized boolean insertESNoID(String index, String type, String json) {
    ...
    public static void batchInsertEs(Map<String, String> map, String index, String type) {
    ...
    public void updateES( String index, String type, String id, JSONObject data) {
     ...
     * 根据属性值查询删除
     */
    public void deleteByQuery( String index, String type, String key, String value) {
     ...
     * 根据属性值查询删除
     */
    public void updateByQuery(String index, String key, String value) {
    ...
    public static void batchUpdateEs(String index, Map<String, String> queryMap, String updateKey, String updateValue) {
    ...
    public static void batchInsertEs(List<String> list) {
    ...
    public synchronized String searchRoutingId(String index, String type, String id,String routing) {
